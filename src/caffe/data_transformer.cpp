...
template<typename Dtype>//--------ininin 
void DataTransformer<Dtype>::Transform(const Datum& datum,
  ...
  if (datum.encoded()) {
	...
	LOG(INFO) << "---hello in transfrom to dig cropbox :  JUMP TO ELSE";
    if (param_.force_color() || param_.force_gray()) {
  ...
  if (anno_datum.type() == AnnotatedDatum_AnnotationType_BBOX) {
    ...
    for (int g = 0; g < anno_datum.annotation_group_size(); ++g) {
      ...
      for (int a = 0; a < anno_group.annotation_size(); ++a) {
		...
		//LOG(INFO) << "resize_bbox in TransformAnnotation: (" << resize_bbox.xmin() << "," << resize_bbox.ymin() << ")";
        ...
        if (param_.has_emit_constraint() &&
		  ...
		  LOG(INFO) << "MeetEmitConstraint : continue";
          continue;
        }
...
void DataTransformer<Dtype>::Transform(const cv::Mat& cv_img,
                                       ...
                                       bool* do_mirror) {
  // Check dimensions.
  //LOG(INFO) << "---hello in transfrom to dig cv img :  JUMP TO cv_img";
  const int img_channels = cv_img.channels();
  const int channels = transformed_blob->channels();
  const int height = transformed_blob->height();
  const int width = transformed_blob->width();
  const int num = transformed_blob->num();
  //LOG(INFO) << "imgchannels , channels, height, width, num " << img_channels << "|" << channels << "|" << height << "|" << width << "|" << num;
  ...
  *do_mirror = param_.mirror() && Rand(2);
  ...
  //cv::imshow("crop_img", cv_cropped_image);
  //cv::waitKey(0);
  // Return the normalized crop bbox.
