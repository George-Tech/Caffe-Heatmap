...
bool ProjectBBox(const NormalizedBBox& src_bbox, const NormalizedBBox& bbox, //src_bbox=crop_bbox[0 0 1 1], bbox=resize_bbox, &proj_bbox
                 NormalizedBBox* proj_bbox) {
  if (bbox.xmin() >= src_bbox.xmax() || bbox.xmax() <= src_bbox.xmin() ||
      bbox.ymin() >= src_bbox.ymax() || bbox.ymax() <= src_bbox.ymin()) {
	  //LOG(INFO) << "ProjectBBox : crop_bbox box : " << src_bbox.xmin() << " " << src_bbox.ymin() <<" " << src_bbox.xmax() << " " << src_bbox.ymax();
	  //LOG(INFO) << "ProjectBBox : resize_bbox box : " << bbox.xmin() << " " << bbox.ymin() <<" " << bbox.xmax() << " " << bbox.ymax();
	  //LOG(INFO) << "ProjectBBox  false [1] ";
    return false;
  }
  //LOG(INFO) << "ProjectBBox : src box : " << src_bbox.xmin() << " " << src_bbox.ymin() <<" " << src_bbox.xmax() << " " << src_bbox.ymax();
  float src_width = src_bbox.xmax() - src_bbox.xmin();//1
  float src_height = src_bbox.ymax() - src_bbox.ymin();//1
  ...
  //LOG(INFO) << "ProjectBBox : project box : " << proj_bbox->xmin() << " " << proj_bbox->ymin() <<" " << proj_bbox->xmax() << " " << proj_bbox->ymax();
  ClipBBox(*proj_bbox, proj_bbox);
  if (1) {//(BBoxSize(*proj_bbox) > 0) {
    return true;
  } else {
	LOG(INFO) << "ProjectBBox  false [2] ";
    return false;
  }
}

void ExtrapolateBBox(const ResizeParameter& param, const int height,
    const int width, const NormalizedBBox& crop_bbox, NormalizedBBox* bbox) { // bbox=transformed_bbox
...
bool MeetEmitConstraint(const NormalizedBBox& src_bbox,
                        ...
                        const EmitConstraint& emit_constraint) { //src_bbox=crop_bbox, bbox=resize_bbox,
